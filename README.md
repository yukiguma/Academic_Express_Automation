...
拡張機能は1日1回、GitHub Releasesで新しい安定版が公開されているか確認します。新しいバージョンがある場合は拡張機能アイコンに `NEW` と表示し、ポップアップから最新版の配布zipを直接ダウンロードできます。

ダウンロードしたzipを展開して現在の拡張機能フォルダの内容を置き換え、`chrome://extensions` または `edge://extensions` から拡張機能を再読み込みしてください。更新確認に失敗しても、自動入力などの既存機能には影響しません。

更新確認のため、拡張機能は `api.github.com` に現在公開されている最新バージョンを問い合わせます。認証情報やAcademic Expressのデータは送信しません。
