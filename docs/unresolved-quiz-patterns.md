...
- Dictation 入力前には、進捗表示だけでなく画面本文に現在問の入力対象文字列が出ていることを待つ。進捗だけ先に変わった遷移中状態へキー入力すると、前問または未準備の listener に拾われてミス扱いになる。
- 高速モードでも Dictation は `SPEED_CONFIG` の `DICTATION_PRE_SOLVE_WAIT` で入力開始前待機、`DICTATION_AUTO_SOLVE_RESUME_WAIT` で複数問の自動再開待機を設定する。通常の自動遷移問題より画面進捗と入力受付の準備にズレが出やすく、50ms 再開では遷移待機中に次問入力を始める可能性がある。
