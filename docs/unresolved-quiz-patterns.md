...
- Dictation 入力前には、進捗表示だけでなく Dictation DOM が現在問として入力可能な状態であることを待つ。全文表示型は入力対象文字列の表示、prefix/空欄型は hidden box の残存で判定する。prefix が正答データに含まれず未入力部分より長い問題もあるため、表示文字数と正答文字数は比較しない。未準備なら入力せずに同じ問題を再試行する。進捗だけ先に変わった遷移中状態へキー入力すると、前問または未準備の listener に拾われてミス扱いになる。
- Dictation は key event を送っただけでは成功扱いにしない。1文字ごとに Dictation DOM の表示文字または hidden box 数が変化したことを確認し、変化しなければ入力を中断して同じ問題を再試行する。
- Dictation の再試行時は、最初の hidden box より前にある入力済み `FontBox` が正答の先頭と一致すれば、その文字数を除いた未入力部分だけを送る。途中まで受理された後に一時的な入力失敗が起きても、正答の先頭から送り直して現在カーソルと不一致にならないようにする。
