...
- `VocabrarySpelling3` では `[Both] [of]` のように複数 bracket が連続し、正答文字列上は単語間スペースを保持する。solver は bracket 群を `Both of` のように結合してから、`FontBox` へ送る keyboard event では空白を除いた英数字だけを入力する。
- spelling では `[post office]` のように1つの bracket 内にスペースを含む正答がある。画面側の `FontBox` は空白を区切りとして扱い、keyboard event では英数字だけを受け付けるため、solver は正答文字列を保持したまま `postoffice` のように空白を除いた入力列を送る。
